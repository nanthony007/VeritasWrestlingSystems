...
def savecsv():
    ...
    match_df['PassiveDiff'] = match_df.apply(
        lambda x: x.Passive - x.oPassive, axis=1)
    ...
    match_df['BinaryResult'] = [1 if row >
                                1 else 0 for row in match_df.NumResult.values]
    match_df['BinaryResultText'] = ['Win' if row >
                                    1 else 'Loss' for row in match_df.NumResult.values]
    match_df['ResultType'] = [find_result_types(
        row) for row in match_df.Result.values]
