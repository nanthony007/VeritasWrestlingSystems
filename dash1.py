...
def update_graph1(dropdown_names, **kwargs): 
    user_qs = kwargs['user']
    u = User.objects.get(username=user_qs)
    r = u.profile.roster.filter(name__in=dropdown_names)
    print(r)
    match_qs = []
    for wrestler in r:
    ...
    traces = []
    for w in matches.focus_id.unique():
        traces.append(go.Scatter(
            x=matches[matches['focus_id']==w]['result'],
            y=matches[matches['focus_id']==w]['npf'],
            text=matches[matches['focus_id']==w]['focus_id'].iloc[0],
            mode='markers',
            opacity=0.7,
            marker={
                'size': 10,
